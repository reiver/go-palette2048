...
func TestSlice_Color(t *testing.T) {

	var buffer [palette2048.ByteSize]uint8 = [palette2048.ByteSize]uint8{
...

	{
		...
		for index:=0; index<length; index++ {
			...
			if nil == color {
				t.Errorf("For index=%d, received nil color.", index)
				continue
			}
			...
			{
				offsetR := index*rgba32.ByteSize + 0
				offsetG := index*rgba32.ByteSize + 1
				offsetB := index*rgba32.ByteSize + 2
				offsetA := index*rgba32.ByteSize + 3

				eR = uint32(buffer[offsetR]) * (0xffff/0xff)
				eG = uint32(buffer[offsetG]) * (0xffff/0xff)
				eB = uint32(buffer[offsetB]) * (0xffff/0xff)
				eA = uint32(buffer[offsetA]) * (0xffff/0xff)
			}

			if eR != aR || eG != aG || eB != aB || eA != aA {
				t.Errorf("For index=%d, the actual color was not what was expected.", index)
