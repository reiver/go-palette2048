...
func TestSlice_Index(t *testing.T) {
	...
	for testNumber, test := range tests {
		...
		actual := palette.Index(c)

		expected := test.Expected

		if expected != actual {
			t.Errorf("For test #%d, the actual index was not what was expected.", testNumber)
			t.Logf("COLOR:   rgba() -> (0x%02X,0x%02X,0x%02X,%d)", test.R, test.G, test.B, test.A)
