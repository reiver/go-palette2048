...
func TestSlice_Convert(t *testing.T) {
	...
	for testNumber, test := range tests {
		...
		actualColor := palette.Convert(c)
		if nil == actualColor {
			t.Errorf("For test #%d, received nil color.", testNumber)
			continue
		}

		aR, aG, aB, aA := actualColor.RGBA()

		eR := uint32(test.ExpectedR) * (0xffff/0xff)
		eG := uint32(test.ExpectedG) * (0xffff/0xff)
		eB := uint32(test.ExpectedB) * (0xffff/0xff)
		eA := uint32(test.ExpectedA) * (0xffff/0xff)

		if eR != aR || eG != aG || eB != aB || eA != aA {
			t.Errorf("For test #%d, the actual color was not what was expected.", testNumber)
			t.Logf("COLOR:   rgba() -> (%d,%d,%d,%d)", test.R, test.G, test.B, test.A)
